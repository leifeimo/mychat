...
public class PdfTemplate {	
	public static void main(String[] args) {
		try {
			...
			String TemplatePDF = "D:/test/PdfTemplate.pdf";//设置模板路径
			FileOutputStream fos = new FileOutputStream("D:/test/Pdftest.pdf");//需要生成PDF
			...
			for (int item = 0; item < page; item++) {
				...
				form.setField("联系人姓名", "蓝飞");//插入的数据都为字符类型
				...
				for (int j = 0; j < pageCount; j++) {
					form.setField("项目/任务", index + "");
					form.setField("星期一", "星期一[" + index + "]");
					form.setField("星期二", "星期二[" + index + "]");
					form.setField("星期三", "星期三[" + index + "]");
					form.setField("星期四", "星期四[" + index + "]");
					form.setField("星期五", "星期五[" + index + "]");
					form.setField("星期六", "星期六[" + index + "]");
					form.setField("星期日[" + j + "]", "星期日[" + index + "]");
					form.setField("意见", "同意[" + j + "]");
					index++;
				}
